...
// Generate README-pt.md from internal/Curriculum-pt.md + dictionary-pt/*.md + internal/README.template-pt.md.
...
const CURRICULUM = join(HERE, "Curriculum-pt.md");
const TEMPLATE = join(HERE, "README.template-pt.md");
const DICT_DIR = join(ROOT, "dictionary-pt");
const OUTPUT = join(ROOT, "README-pt.md");
...
// then replace spaces with hyphens. "Seção 1 — Fundamentos" → "seção-1--fundamentos".
  ...
  text.split("\n").forEach((raw, idx) => {
    ...
    if (line.startsWith("## ")) {
      if (!SECTION_RE.test(line)) {
        fail(
          `Curriculum-pt.md:${lineNo}: section heading must match "## Seção N — Title" (em-dash required): ${line}`
        );
      }
    ...
    if (line.startsWith("- ")) {
      if (!current)
        fail(`Curriculum-pt.md:${lineNo}: bullet before any section heading`);
      const m = line.match(BULLET_RE);
      if (!m || !m[1])
        fail(`Curriculum-pt.md:${lineNo}: malformed bullet: ${line}`);
      const term = m[1];
      if (term.trim() !== term)
        fail(`Curriculum-pt.md:${lineNo}: term has surrounding whitespace`);
      if (/[*_`\[]/.test(term))
        fail(
          `Curriculum-pt.md:${lineNo}: term must be plain text, no markdown: ${term}`
        );
    ...
    fail(
      `Curriculum-pt.md:${lineNo}: only "## Seção N — Title" headings and "- Term" bullets are allowed: ${line}`
    );
  });
...
function main(): void {
  ...
  for (const section of sections) {
    ...
    for (const term of section.terms) {
      if (seen.has(term)) fail(`Curriculum-pt.md: duplicate term "${term}"`);
      seen.add(term);
      ...
      try {
        ...
        fail(
          `Curriculum-pt.md references "${term}" but ${entryPath} does not exist`
        );
      }
      parts.push(
        `### ${term}`,
  ...
  if (orphans.length)
    fail(
      `dictionary-pt/ entries not referenced by Curriculum-pt.md: ${orphans.join(", ")}`
    );
