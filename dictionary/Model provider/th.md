...
ระบบใดก็ตามที่ทำหน้าที่เปิดให้ใช้งาน [model](./Model.md) เพื่อการ[อนุมาน](./Inference.md) โดยทั่วไปจะอยู่ในรูปของบริการคลาวด์ทางไกล (เช่น Anthropic, OpenAI, Google) แต่ก็สามารถรันแบบโลคัลในเครื่องของตัวเองได้เช่นกัน เช่น Ollama, LM Studio หรือ llama.cpp ตัวระบบควบคุม ([harness](./Harness.md)) ไม่ได้ทำหน้าที่รันโมเดลด้วยตัวเอง แต่มันจะส่งคำสั่งไปให้ตัว provider เป็นผู้ประมวลผลแทน

ฝั่ง provider จะเป็นผู้ถือครองระบบเครื่องจักรทั้งหมด โดยค่า [parameters](./Parameters.md) จะจัดเก็บอยู่บนฮาร์ดแวร์ของพวกเขา และทุก ๆ [model provider request](./Model%20provider%20request.md) ก็คือการที่ระบบ harness ส่ง [token](./Token.md) ผ่านเครือข่ายเน็ตเวิร์กไปให้ แล้วรอรับผลการคาดเดากลับมา ด้วยเหตุนี้ ตัว provider จึงเป็นต้นตอของปัญหาหลาย ๆ อย่างที่คนมักจะเข้าใจผิดไปโทษตัว model หรือตัว harness เช่น ปัญหาการจำกัดโควตาการเรียกใช้งาน (rate limits) ประสิทธิภาพระบบที่ดรอปหล่นลง หรือระบบล่ม (outages) ดังนั้นเมื่อตัว [agent](./Agent.md) เกิดอาการค้างกลางคันระหว่างใช้งาน ([session](./Session.md)) หรือแจ้งข้อผิดพลาดในทุก ๆ รอบสนทนา ([turn](./Turn.md)) สิ่งแรกที่ควรทำคือการเข้าไปเช็กหน้าสถานะระบบ (status page) ของผู้ให้บริการก่อนสิ่งอื่นใด

นอกจากนี้ provider ยังเป็นผู้กำหนดเงื่อนไขทางการค้าทั้งหมด ไม่ว่าจะเป็นการคิดราคาต่อหนึ่งโทเค็นแยกกันระหว่าง [input](./Input%20tokens.md) และ [output tokens](./Output%20tokens.md) ส่วนลดจากการใช้ [prefix cache](./Prefix%20cache.md) รวมถึงการกำหนดว่าจะมีโมเดลรุ่นใดให้เลือกใช้บ้าง พึงระลึกไว้ว่าผู้ให้บริการระบบ (provider) กับผู้พัฒนาตัวโมเดลอาจเป็นคนละบริษัทกันก็ได้ เช่น บริการ Amazon Bedrock, Google Vertex AI หรือ OpenRouter ที่นำโมเดลของค่ายอื่นมาเปิดให้บริการผ่านระบบของตน
