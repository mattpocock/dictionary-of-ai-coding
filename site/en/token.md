...
The atomic unit a [model](/en/model) reads and writes. Roughly word-sized but not exactly — common words are one token, rare or long ones split into several. [Context window](/en/context-window) size, cost, and latency are all counted in tokens.

Text becomes tokens via a tokenizer: a fixed vocabulary of tens of thousands of fragments, learned before [training](/en/training), that splits any input into a sequence of vocabulary entries. The model never sees characters or words — every piece of text is converted to tokens on the way in, and [next-token prediction](/en/next-token-prediction) produces output one token at a time on the way out.
...
Tokens are the unit everything else is measured in. Cost is per token — providers bill [input tokens](/en/input-tokens) and [output tokens](/en/output-tokens) separately. Speed is tokens per second, since output is generated one token at a time. And the context window is a fixed number of tokens, so the token count of your files decides how much fits.
...
<div style="text-align:center;margin-top:2rem">
  <a href="/th/token" class="VPLink">Read in ไทย →</a>
</div>
