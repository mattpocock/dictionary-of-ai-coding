...
จำนวน [input tokens](/th/input-tokens) ที่ทางฝั่งผู้ให้บริการ ([model provider](/th/model-provider)) ได้ทำการบันทึกและจัดเก็บไว้จากการส่งคำขอ ([model provider request](/th/model-provider-request)) ในรอบก่อนหน้า ทำให้ไม่ต้องสิ้นเปลืองพลังงานสมองกลับไปประมวลผลคำนวณซ้ำอีกครั้ง เมื่อคำขอที่ถูกส่งเข้ามาติดต่อกันมีข้อความส่วนหัวที่ร่วมกัน (shared prefix) ตัวผู้ให้บริการจะดึงผลงานชิ้นเดิมกลับมาใช้ซ้ำทันทีผ่านกลไก [prefix cache](/th/prefix-cache) และปรับเปลี่ยนสถานะบิลของข้อความส่วนที่ถูกแคชไว้เหล่านั้นให้อยู่ในราคาที่ถูกลงอย่างมาก นี่คือกลไกหลักชิ้นสำคัญที่ช่วยให้การเปิดใช้งาน [session](/th/session) ระยะยาวเป็นเรื่องที่เอื้อมถึงในแง่ของงบประมาณ เพราะหากไม่มีมัน ทุก ๆ รอบของการพูดคุย ([turn](/th/turn)) คุณจะต้องควักเงินจ่ายค่าประวัติย้อนหลังทั้งหมดเต็มราคาซ้ำแล้วซ้ำเล่า

เหตุผลที่เรื่องนี้มีความสำคัญอย่างยิ่งมาจากวิธีการคำนวณบิลค่าบริการของตัวระบบ เนื่องจากตัว [model](/th/model) ทำงานแบบ [stateless](/th/stateless) (ไม่มีการเก็บจำสถานะ) ทุก ๆ คำขอจึงต้องแบกเอาเนื้อหาบทสนทนาทั้งหมดตั้งแต่แรกเริ่ม ไม่ว่าจะเป็นตัว [system prompt](/th/system-prompt) ข้อความคุยโต้ตอบทุกประโยค รวมถึงผลลัพธ์จากเครื่องมือทุกชิ้น ([tool result](/th/tool-result)) ส่งกลับไปใหม่ทั้งหมดในฐานะโทเค็นขาเข้า เมื่อการสนทนาเดินทางไปถึง turn ที่ 50 แต่ละคำขอจะแบกประวัติย้อนหลังหนาถึง 50 รอบ และคุณจะต้องจ่ายค่าบริการราคาเต็มให้กับข้อมูลชุดเดิมเหล่านั้นในทุก ๆ ครั้ง ระบบแคชจึงเข้ามาเปลี่ยนสูตรคณิตศาสตร์ในบิลใหม่ โดย [token](/th/token) ตัวใดก็ตามที่ผู้ให้บริการเคยประมวลผลผ่านตามาแล้วในข้อความส่วนหัวที่เหมือนเดิมเป๊ะ จะถูกเปลี่ยนไปคิดเงินในฐานะ cache tokens ซึ่งมักจะมีราคาถูกลงเหลือเพียง 1 ใน 10 ของราคาขาเข้าปกติหรือน้อยกว่านั้น ใน session ยาว ๆ ข้อมูลเกือบทั้งหมดที่คุณส่งไปจะถูกนับเป็น cache tokens ช่วยให้บิลตอนสิ้นเดือนไม่พุ่งทะลุเพดานไปไกล
...
<div style="text-align:center;margin-top:2rem">
  <a href="/en/cache-tokens" class="VPLink">Read in English →</a>
</div>
