...
มีการเก็บรักษาและส่งต่อข้อมูลไปข้างหน้า ตัว [session](/th/session) จะมีคุณสมบัติแบบ stateful ข้ามแต่ละรอบสนทนา ([turns](/th/turn)) ข้อมูล[บริบท](/th/context)จะค่อย ๆ สะสมหนาขึ้นเรื่อย ๆ ในระหว่างที่เซสชันรันไป ซึ่งเป็นสาเหตุที่ทำให้เซสชันที่ลากยาวเกินไปเริ่มหลุดเข้าไปอยู่ใน [โซนเอ๋อ](/th/smart-zone) (dumb zone) นอกจากนี้เรายังสามารถสร้าง [agent](/th/agent) ให้มีคุณสมบัติแบบ stateful ข้าม **เซสชัน** ได้ โดยการเพิ่ม [ระบบความจำ](/th/memory-system) (memory system) เข้าไปทำหน้าที่จัดเก็บข้อมูลลงสู่สภาพแวดล้อมระบบ ([environment](/th/environment)) และโหลดข้อมูลเก่าตัวนั้นกลับเข้ามาใหม่ในตอนเริ่มต้นเซสชันถัดไป พึงระลึกไว้ว่าตัว [model](/th/model) เองไม่มีวันทำงานแบบ stateful ได้เลย ความต่อเนื่องทั้งหมดที่เห็นเป็นเพียงฝีมือของระบบควบคุม ([harness](/th/harness)) ที่ช่วยส่งบริบทซ้ำเข้าไปให้แทน เป็นคำตรงข้ามกับ [stateless](/th/stateless)

การจัดเก็บสถานะ (state) ในแต่ละเลเยอร์ของระบบ:

| เลเยอร์ระบบ | มีสถานะแบบ Stateful ไหม? | รูปแบบการทำงานและกลไก                                                                                                    |
| ----------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| Model       | ไม่เคยมี (Never)         | ค่าพารามิเตอร์ ([parameters](/th/parameters)) ถูกแช่แข็งถาวร มันรับรู้ข้อมูลเฉพาะแค่สิ่งที่แนบมาในแต่ละ request เท่านั้น |
| Session     | มีข้ามรอบสนทนา (turns)   | ตัว harness จะคอยนำทุกข้อความและผลลัพธ์ของเครื่องมือ ([tool result](/th/tool-result)) ไปแนบต่อท้ายบริบทเสมอ              |
...
<div style="text-align:center;margin-top:2rem">
  <a href="/en/stateful" class="VPLink">Read in English →</a>
</div>
